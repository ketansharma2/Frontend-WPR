...
const RnR = () => {
  ...
  const fetchRnrData = async () => {
    try {
      const token = localStorage.getItem("token");
      if (!token) return;

      const response = await fetch(`${API_BASE_URL}/rnr`, {
        headers: {
  ...
  const fetchFixedTasks = async () => {
    try {
      ...
      const response = await fetch(`${API_BASE_URL}/fixed-tasks`, {
        headers: {
          "Authorization": `Bearer ${token}`,
          "Content-Type": "application/json"
        }
  ...
  const handleFixedTaskSubmit = async () => {
    try {
      ...
      const method = isEditing ? 'PUT' : 'POST';
      const url = isEditing ? `${API_BASE_URL}/fixed-tasks/${editingFixedTask.fixed_task_id}` : `${API_BASE_URL}/fixed-tasks`;
