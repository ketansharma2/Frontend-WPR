...
const SubAdminRnR = () => {
  ...
  useEffect(() => {
    ...
    if (profile) {
      ...
      setFixedTaskForm(prev => ({ ...prev, assignTo: parsed.name || 'Sub Admin', assignedBy: parsed.name || 'Sub Admin' }));
  ...
  const fetchTeamMembers = async (profile) => {
    try {
      if (profile.user_type === 'Admin') {
        // For admin, use the same API as AdminHome
        const data = await api.getAssignMembers();
        const members = data.members || [];
        setTeamMembers(members);
      } else {
        // For sub-admin, use sub-admin API
  ...
  const handleFixedTaskSubmit = async () => {
    try {
      ...
      let targetUserId = userProfile?.user_id; // Default to Sub Admin
      if (fixedTaskForm.assignTo !== (userProfile?.name || 'Sub Admin')) {
      ...
      if (response.ok) {
        ...
        setFixedTaskForm({
          ...
          frequency: 'Daily',
          assignTo: '',
          assignedBy: userProfile?.name || 'Sub Admin'
        });
            ...
            <div className="popup-content">
              <div className="form-grid-container">
                ...
                <div className="field-box span-6">
                  ...
                  >
                    <option value={userProfile?.name || 'Sub Admin'}>Assign to Me</option>
                    {teamMembers.map(member => (
